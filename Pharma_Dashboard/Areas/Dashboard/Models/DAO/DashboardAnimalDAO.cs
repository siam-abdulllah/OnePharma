...
{
    public class DashboardAnimalDAO : ReturnData
    {
        ...
        public HomeDashboard GetDashboardData(string toDate)
        {
            try
            {
                ...
                else if (ACCESS_LEVEL == "D")
                {
                    ...
                    row = dbHelper.GetDataTable(dbConnection.SAConnStrReader("DashboardAnimal"), Depot_Name_Qry).Rows[0];
                ...
                homeDashboard.LM_Collection_Amount = GetLMCollection(accessLevelParam, toDate);
                homeDashboard.UpTo_Month_Collection = GetUpToMonthCollection(accessLevelParam, firstDayMonth, toDate);
                homeDashboard.LM_UpTo_Month_Collection = GetLMUpToMonthCollection(accessLevelParam, toDate);
                // homeDashboard.TotalCustomer = GetTotalCustomer(accessLevelParam);
                homeDashboard.TotalDues = GetTotalDues(accessLevelParam, toDate);
        ...
        public Target GetTargetCurrentMonth(string accessLevelParam, string YYYYMM)
        {
            try
            {
                ...
                dt = dbHelper.GetDataTable(dbConnection.SAConnStrReader("DashboardAnimal"), target_Current_Month_Qry);
                if (dt.Rows.Count > 0)
                {
                    row = dt.Rows[0];
        ...
        public TodaySale GetTodaySales(string accessLevelParam, string toDate)
        {
            ...
            try
            {
                string today_Sales_Qry =
                    ...
                    " from ( select MOP, SUM(INVOICE_AMT) NET_INV_AMT_CA, 0 NET_INV_AMT_CR  from DATE_WISE_SALES where TO_CHAR(INVOICE_DATE,'dd/MM/YYYY') = '" + toDate + "' AND MOP = 'CA'  " +
                    accessLevelParam + " " +
                    " GROUP BY  MOP  UNION ALL select MOP, 0 NET_INV_AMT_CA, SUM(INVOICE_AMT) NET_INV_AMT_CR  from DATE_WISE_SALES where  TO_CHAR(INVOICE_DATE,'dd/MM/YYYY')  = '" + toDate + "' AND MOP = 'CR' " +
                    accessLevelParam + " " +
                    " GROUP BY  MOP )";
                TodaySale todaySale = new TodaySale();
                dt = dbHelper.GetDataTable(dbConnection.SAConnStrReader("DashboardAnimal"), today_Sales_Qry);
                if (dt.Rows.Count > 0)
                {
                    row = dt.Rows[0];
        ...
        public UpToMonthSale GetUpToMonthSales(string accessLevelParam, string firstDayMonth, string toDate)
        {
            try
            {
                string upToMonth_Sales_Amnt_Qry =
                   ...
                   " from ( select MOP, SUM(INVOICE_AMT) NET_INV_AMT_CA, 0 NET_INV_AMT_CR  from DATE_WISE_SALES where INVOICE_DATE between TO_DATE('" + firstDayMonth + "','dd/MM/YYYY') AND TO_DATE('" + toDate + "','dd/MM/YYYY')  AND MOP = 'CA' " +
                   accessLevelParam + " " +
                   " GROUP BY  MOP  UNION ALL select MOP, 0 NET_INV_AMT_CA, SUM(INVOICE_AMT) NET_INV_AMT_CR  from DATE_WISE_SALES where INVOICE_DATE between TO_DATE('" + firstDayMonth + "','dd/MM/YYYY') AND TO_DATE('" + toDate + "','dd/MM/YYYY')  AND MOP = 'CR' " +
                   accessLevelParam + " " +
                   " GROUP BY  MOP )";

                UpToMonthSale upToMonthSale = new UpToMonthSale();
                dt = dbHelper.GetDataTable(dbConnection.SAConnStrReader("DashboardAnimal"), upToMonth_Sales_Amnt_Qry);
                if (dt.Rows.Count > 0)
                {
                    row = dt.Rows[0];
                    //row = dbHelper.GetDataTable(dbConnection.SAConnStrReader("DashboardAnimal"), upToMonth_Sales_Amnt_Qry).Rows[0];
        ...
        public LMUpToDate GetLMUpToDateSales(string accessLevelParam, string toDate)
        {
            try
            {
                string upToMonth_Sales_Amnt_Qry =
                    ...
                    " from ( select MOP, SUM(INVOICE_AMT) NET_INV_AMT_CA, 0 NET_INV_AMT_CR  from DATE_WISE_SALES where INVOICE_DATE between TRUNC(ADD_MONTHS(TO_DATE('" + toDate + "','dd/MM/YYYY'), -1),'MM') AND TRUNC(add_months(TO_DATE('" + toDate + "','dd/MM/YYYY'), -1))  AND MOP = 'CA' " +
                    accessLevelParam + " " +
                    " GROUP BY  MOP  UNION ALL select MOP, 0 NET_INV_AMT_CA, SUM(INVOICE_AMT) NET_INV_AMT_CR  from DATE_WISE_SALES where INVOICE_DATE between TRUNC(ADD_MONTHS(TO_DATE('" + toDate + "','dd/MM/YYYY'), -1),'MM') AND TRUNC(add_months(TO_DATE('" + toDate + "','dd/MM/YYYY'), -1))  AND MOP = 'CR' " +
                    accessLevelParam + " " +
                    " GROUP BY  MOP )";
        ...
        public TotalMPO GetTotalMpo(string accessLevelParam, string toDate)
        {
            try
            {
                string Total_Mpo_Qry = "SELECT SUM(DISTINCT LM_MPO) TOTAL_LM_MPO,SUM(DISTINCT CM_MPO) TOTAL_CM_MPO,SUM(TOTAL_ACTIVE_MPO) TOTAL_ACTIVE_MPO  FROM( " +
                                       " SELECT 0 LM_MPO,COUNT(DISTINCT MPO_CODE) CM_MPO,0 TOTAL_ACTIVE_MPO FROM DATE_WISE_SALES WHERE 1=1  AND TO_CHAR(INVOICE_DATE,'YYYYMM')=TO_CHAR(TO_DATE('" + toDate + "','dd/MM/YYYY'),'YYYYMM') " +
                                       accessLevelParam + " " +
                                       " UNION ALL " +
                                       " SELECT COUNT(DISTINCT MPO_CODE) LM_MPO,0 CM_MPO,0 TOTAL_ACTIVE_MPO FROM DATE_WISE_SALES WHERE 1=1    AND TO_CHAR(INVOICE_DATE,'YYYYMM')=TO_CHAR(TRUNC(add_months(TO_DATE('" + toDate + "','dd/MM/YYYY'), -1)),'YYYYMM') " +
                                       accessLevelParam + " " +
                                       " UNION ALL " +
                ...
                dt = dbHelper.GetDataTable(dbConnection.SAConnStrReader("DashboardAnimal"), Total_Mpo_Qry);
                if (dt.Rows.Count > 0)
                {
                    row = dt.Rows[0];
                    //row = dbHelper.GetDataTable(dbConnection.SAConnStrReader("DashboardAnimal"), Total_Mpo_Qry).Rows[0];
        ...
        public string GetToDayCollection(string accessLevelParam, string toDate)
        {
            try
            {
                ...
                string today_Collection_Amnt_Qry =
                    ...
                    accessLevelParam + " ";
                dt = dbHelper.GetDataTable(dbConnection.SAConnStrReader("DashboardAnimal"), today_Collection_Amnt_Qry);
                if (dt.Rows.Count > 0)
                {
                    row = dt.Rows[0];
                    //row = dbHelper.GetDataTable(dbConnection.SAConnStrReader("DashboardAnimal"), today_Collection_Amnt_Qry).Rows[0];
        ...
        public string GetLMCollection(string accessLevelParam, string toDate)
        {
            try
            {
                string LM_COLLECTION_AMT = "";
                //string today_Collection_Amnt_Qry =
                //    "SELECT ROUND (SUM (COLLECTION_AMT) / 100000, 2) LM_COLLECTION_AMT FROM DATE_WISE_COLLECTION WHERE TO_CHAR(COLLECT_DATE,'RRRRMM') = TO_CHAR(ADD_MONTHS(TO_DATE('" + toDate + "','dd/MM/RRRR'), -1), 'RRRRMM') " +
                string today_Collection_Amnt_Qry =
                    "SELECT ROUND (SUM (COLLECTION_AMT) / 100000, 2) LM_COLLECTION_AMT FROM DATE_WISE_COLLECTION WHERE COLLECT_DATE BETWEEN TRUNC(ADD_MONTHS(TO_DATE ('" + toDate + "','dd/MM/RRRR'),-1),'MM') AND last_day(add_months(TO_DATE('" + toDate + "','dd/MM/YYYY'),-1))  " +
                    accessLevelParam + " ";
                dt = dbHelper.GetDataTable(dbConnection.SAConnStrReader("DashboardAnimal"), today_Collection_Amnt_Qry);
                if (dt.Rows.Count > 0)
                {
                    row = dt.Rows[0];
                    //row = dbHelper.GetDataTable(dbConnection.SAConnStrReader("DashboardAnimal"), today_Collection_Amnt_Qry).Rows[0];
                    LM_COLLECTION_AMT = row["LM_COLLECTION_AMT"].ToString();
                }

                return LM_COLLECTION_AMT;
            }
            catch (Exception e)
            {
                Console.WriteLine(e);
        ...
        public string GetUpToMonthCollection(string accessLevelParam, string firstDayMonth, string toDate)
        {
            try
            {
                ...
                string upToMonth_Collection_Amnt_Qry =
                    ...
                    accessLevelParam + " ";
                dt = dbHelper.GetDataTable(dbConnection.SAConnStrReader("DashboardAnimal"), upToMonth_Collection_Amnt_Qry);
                if (dt.Rows.Count > 0)
                {
                    row = dt.Rows[0];
        ...
        public string GetLMUpToMonthCollection(string accessLevelParam, string toDate)
        {
            try
            {
                string LM_UPTO_COLLECTION_AMT = "";
                string upToMonth_Collection_Amnt_Qry = "SELECT ROUND (SUM (COLLECTION_AMT) / 100000, 2) LM_UPTO_COLLECTION_AMT " +
                    "FROM DATE_WISE_COLLECTION " +
                    "WHERE COLLECT_DATE BETWEEN TRUNC(ADD_MONTHS (TO_DATE ('" + toDate + "','dd/MM/YYYY'),-1),'MM') AND " +
                    "TRUNC(ADD_MONTHS (TO_DATE ('" + toDate + "','dd/MM/YYYY'),-1))" +
                    accessLevelParam + " ";
                ...
                {
                    row = dt.Rows[0];
                     LM_UPTO_COLLECTION_AMT = row["LM_UPTO_COLLECTION_AMT"].ToString();
                }

                return  LM_UPTO_COLLECTION_AMT;
                ;
            }
            catch (Exception e)
            {
        ...
        public TotalCustomer GetTotalCustomer(string accessLevelParam, string toDate)
        {
            try
            {
                ...
                dt = dbHelper.GetDataTable(dbConnection.SAConnStrReader("DashboardAnimal"), Total_Customer_Qry);
                if (dt.Rows.Count > 0)
                {
                    row = dt.Rows[0];
                    // row = dbHelper.GetDataTable(dbConnection.SAConnStrReader("DashboardAnimal"), Total_Customer_Qry).Rows[0];
        ...
        public TotalDue GetTotalDues(string accessLevelParam, string toDate)
        {
            try
            {
                string Matured_Dues_Qry = "SELECT ROUND (SUM (DUES_AMT_CA) / 100000, 2) TOTAL_DUES_AMT_CA," +
                                        "       ROUND (SUM (DUES_AMT_CR) / 100000, 2) TOTAL_DUES_AMT_CR," +
                                        "       ROUND (" +
                                        "          (SUM (DUES_AMT_CA) + SUM (DUES_AMT_CR)) / 100000," +
                                        "          2)" +
                                        "          TOTAL_DUES_AMT" +
                                        "  FROM (  SELECT MODE_OF_PAYMENT," +
                                        "                 SUM (DUES_AMT) DUES_AMT_CA," +
                                        "                 0 DUES_AMT_CR" +
                                        "            FROM VW_SALES_COLLECTION_DUES_DTL" +
                                        "           WHERE     MODE_OF_PAYMENT = 'CA'" +
                                        "                 AND TO_DATE (INVOICE_DATE, 'dd/MM/YYYY') <= TO_DATE ('" + toDate + "', 'dd/MM/YYYY') " +
                                         accessLevelParam + "" +
                                        "        GROUP BY MODE_OF_PAYMENT" +
                                        "        UNION ALL" +
                                        "          SELECT MODE_OF_PAYMENT," +
                                        "                 0 DUES_AMT_CA," +
                                        "                 SUM (DUES_AMT) DUES_AMT_CR" +
                                        "            FROM VW_SALES_COLLECTION_DUES_DTL" +
                                        "           WHERE     MODE_OF_PAYMENT = 'CR'" +
                                        "                 AND TO_DATE (INVOICE_DATE, 'dd/MM/YYYY') <= TO_DATE ('" + toDate + "', 'dd/MM/YYYY') " +
                                         accessLevelParam + "" +
                                        "        GROUP BY MODE_OF_PAYMENT)";

                TotalDue totalDue = new TotalDue();
                dt = dbHelper.GetDataTable(dbConnection.SAConnStrReader("DashboardAnimal"), Matured_Dues_Qry);
                if (dt.Rows.Count > 0)
                {
                    row = dt.Rows[0];
                    //row = dbHelper.GetDataTable(dbConnection.SAConnStrReader("DashboardAnimal"), Matured_Dues_Qry).Rows[0];
                    totalDue.Total_Due = row["TOTAL_DUES_AMT"].ToString();
                    totalDue.Total_Due_CA = row["TOTAL_DUES_AMT_CA"].ToString();
                    totalDue.Total_Due_CR = row["TOTAL_DUES_AMT_CR"].ToString();
                }
        ...
        public ActiveMaturedDue GetActiveMaturedDues(string accessLevelParam)
        {
            try
            {
                ...
                dt = dbHelper.GetDataTable(dbConnection.SAConnStrReader("DashboardAnimal"), Active_Matured_Dues_Qry);
                if (dt.Rows.Count > 0)
                {
                    row = dt.Rows[0];
                    //row = dbHelper.GetDataTable(dbConnection.SAConnStrReader("DashboardAnimal"), Active_Matured_Dues_Qry).Rows[0];
        ...
        public DiscontinueMaturedDue GetDiscontinueMaturedDues(string accessLevelParam)
        {
            try
            {
                ...
                dt = dbHelper.GetDataTable(dbConnection.SAConnStrReader("DashboardAnimal"), Discontinue_Matured_Dues_Qry);
                if (dt.Rows.Count > 0)
                {
                    row = dt.Rows[0];
                    //row = dbHelper.GetDataTable(dbConnection.SAConnStrReader("DashboardAnimal"), Discontinue_Matured_Dues_Qry).Rows[0];
        ...
        public ImmaturedDue GetImmaturedDue(string accessLevelParam, string toDate)
        {
            try
            {
                ...
                dt = dbHelper.GetDataTable(dbConnection.SAConnStrReader("DashboardAnimal"), IMMatured_Dues_Qry);
                if (dt.Rows.Count > 0)
                {
                    row = dt.Rows[0];
                    //row = dbHelper.GetDataTable(dbConnection.SAConnStrReader("DashboardAnimal"), IMMatured_Dues_Qry).Rows[0];
        ...
        public ProductValueSale GetProductSalesValue(string accessLevelParam, string YYYYMM, string toDate)
        {
            ...
            try
            {
                ...
                dt = dbHelper.GetDataTable(dbConnection.SAConnStrReader("DashboardAnimal"), Product_value_sales_qry);
                if (dt.Rows.Count > 0)
                {
                    row = dt.Rows[0];
                    //row = dbHelper.GetDataTable(dbConnection.SAConnStrReader("DashboardAnimal"), CM_Product_Value_Sales_Qry).Rows[0];
        ...
        public string GetCommercialStockValuation(string accessLevelParam)
        {
            try
            {
                ...
                dt = dbHelper.GetDataTable(dbConnection.SAConnStrReader("DashboardAnimal"), Commercial_Stock_Valuation_Qry);
                if (dt.Rows.Count > 0)
                {
                    row = dt.Rows[0];
                    //row = dbHelper.GetDataTable(dbConnection.SAConnStrReader("DashboardAnimal"), Commercial_Stock_Valuation_Qry).Rows[0];
        ...
        public string GetCommercialStockValuationDash(string accessLevelParam, string toDate)
        {
            try
            {
                ...
                string Commercial_Stock_Valuation_Qry = "SELECT    ROUND(SUM(NVL(C.UNIT_TP,0) * NVL(A.CLOSING_PASSED_QTY,0))/100000,2) COMMERCIAL_STOCK_VAL                                         " +
                                                        " FROM" +
                                                        "   (" +
                                                        "    SELECT  " +
                                                        "          A.DEPOT_CODE," +
                                                        "          B.STOCK_DATE," +
                                                        "          A.S_PRODUCT_CODE PRODUCT_CODE," +
                                                        "          NVL(A.CLOSING_PASSED_QTY,0) CLOSING_PASSED_QTY" +
                                                        "    FROM  " +
                                                        "        DAILY_STOCK A," +
                                                        "        (       " +
                                                        "         SELECT DEPOT_CODE,S_PRODUCT_CODE,MAX(STOCK_DATE) STOCK_DATE " +
                                                        "         FROM DAILY_STOCK" +
                                                        "         WHERE STOCK_DATE<=TO_DATE('" + toDate + "','DD/MM/RRRR')" +
                                                        "         GROUP BY DEPOT_CODE,S_PRODUCT_CODE                " +
                                                        "        ) B       " +
                                                        "    WHERE   A.STOCK_DATE=B.STOCK_DATE" +
                                                        "    AND     A.DEPOT_CODE=B.DEPOT_CODE" +
                                                        "    AND     A.S_PRODUCT_CODE=B.S_PRODUCT_CODE " +
                                                        "   ) A,DEPOT_PRODUCT B,PRODUCT_PRICE C  " +
                                                        " WHERE   B.PRODUCT_CODE=A.PRODUCT_CODE(+) " +
                                                        " AND     B.PRODUCT_CODE=C.PRODUCT_CODE(+)" +
                                                        " AND     C.STATUS='A'" +
                                                        " AND     B.DEPOT_CODE=A.DEPOT_CODE" +
                                                        //" AND    (B.PRODUCT_CODE=:P_PRODUCT_CODE OR :P_PRODUCT_CODE='ALL')" +
                                                        " AND     B.BRAND_CODE='02'" +
                                                       // " AND    (B.PRODUCT_TYPE=:P_PRODUCT_TYPE OR :P_PRODUCT_TYPE='ALL')" +
                                                        " AND    B.PRODUCT_CODE NOT IN ('S001','S002')";
        ...
        public string GetSampleStockValuation(string accessLevelParam)
        {
            try
            {
                ...
                string Sample_Stock_Valuation_Qry =
                    ...
                    accessLevelParam + " ";
                dt = dbHelper.GetDataTable(dbConnection.SAConnStrReader("DashboardAnimal"), Sample_Stock_Valuation_Qry);
                if (dt.Rows.Count > 0)
                {
                    row = dt.Rows[0];
                    //row = dbHelper.GetDataTable(dbConnection.SAConnStrReader("DashboardAnimal"), Sample_Stock_Valuation_Qry).Rows[0];
        ...
        public string GetPPMStockValuation(string accessLevelParam)
        {
            try
            {
                ...
                string PPM_Stock_Valuation_Qry =
                    ...
                    accessLevelParam + " ";
                //
                dt = dbHelper.GetDataTable(dbConnection.SAConnStrReader("DashboardAnimal"), PPM_Stock_Valuation_Qry);
                if (dt.Rows.Count > 0)
                {
                    row = dt.Rows[0];
                    //row = dbHelper.GetDataTable(dbConnection.SAConnStrReader("DashboardAnimal"), PPM_Stock_Valuation_Qry).Rows[0];
        ...
        public string GetGiftStockValuation(string accessLevelParam)
        {
            try
            {
                ...
                string Gift_Stock_Valuation_Qry =
                    ...
                    accessLevelParam + " ";
                dt = dbHelper.GetDataTable(dbConnection.SAConnStrReader("DashboardAnimal"), Gift_Stock_Valuation_Qry);
                if (dt.Rows.Count > 0)
                {
                    row = dt.Rows[0];
                    //row = dbHelper.GetDataTable(dbConnection.SAConnStrReader("DashboardAnimal"), Gift_Stock_Valuation_Qry).Rows[0];
        ...
        public DCCTotalSale GetDCC_Sale(string accessLevelParam)
        {
            try
            {
                ...
                dt = dbHelper.GetDataTable(dbConnection.SAConnStrReader("DashboardAnimal"), DCC_Sales_Qry);
                if (dt.Rows.Count > 0)
                {
                    row = dt.Rows[0];
                    //row = dbHelper.GetDataTable(dbConnection.SAConnStrReader("DashboardAnimal"), DCC_Sales_Qry).Rows[0];
        ...
        public List<DashboardChart> GetBarChartData(string Param, string firstDayMonth, string toDate)
        {
            ...
            try
            {
                string barChart_Qry = "SELECT DISTINCT RSM_NAME,DWS.RSM_CODE, NET_INV_AMT,REGION_NAME,NULL COLOR FROM (SELECT RSM_CODE, ROUND(SUM (INVOICE_AMT)/ 100000,2) NET_INV_AMT FROM DATE_WISE_SALES " +
                ...
                DataTable BarChartdt = dbHelper.GetDataTable(dbConnection.SAConnStrReader("DashboardAnimal"), barChart_Qry);
        ...
        public ProdSale GetProdSalesData(string Param)
        {
            try
            {
                ...
                dt = dbHelper.GetDataTable(dbConnection.SAConnStrReader("DashboardAnimal"), Prod_Sales_Qry);
                if (dt.Rows.Count > 0)
                {
                    row = dt.Rows[0];
        ...
        public WorldCupOffer GetWorldCupOffer(string Param)
        {
            try
            {
                ...
                dt = dbHelper.GetDataTable(dbConnection.SAConnStrReader("DashboardAnimal"), World_Cup_offer_Qry);
                if (dt.Rows.Count > 0)
                {
                    row = dt.Rows[0];
        ...
        public MPOCreditLimit GetMPOCreditLimit(string Param)
        {
            try
            {
                ...
                dt = dbHelper.GetDataTable(dbConnection.SAConnStrReader("DashboardAnimal"), MPO_Credit_Limit_Qry);
                if (dt.Rows.Count > 0)
                {
                    row = dt.Rows[0];
        ...
        public OldCollection GetOldCollectionData(string accessLevelParam, string firstDayMonth, string toDate)
        {
            try
            {
                ...
                dt = dbHelper.GetDataTable(dbConnection.SAConnStrReader("DashboardAnimal"), Old_Collection_Data_Qry);
