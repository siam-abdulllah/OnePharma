app.controller("myCtrl", function ($scope, $http) {
    ...
    $scope.GetUserListOPA = function () {
        $http({
            method: "GET",
            url: MyApp.rootPath + "UserCreationOPA/GetUserListOPA"
    ...
    var columnUserList = [
        ...
        { name: 'GroupCode', displayName: "GroupCode" },
    ...
    $scope.rowDblClickComp = function (row) {
        $scope.frmUserCreationOPA.Employee = row.entity;
        ...
        $scope.GroupCode = row.entity.GroupCode;
    ...
    $scope.SaveData = function () {
        $scope.SaveDb = {};
        $scope.SaveDb.Username = $scope.frmUserCreationOPA.Employee.EmployeeCode;
        $scope.SaveData.UserId = $scope.UserId;
        $scope.SaveDb.Password = $scope.Password;
        $scope.SaveDb.Status = $scope.Status;
        $scope.SaveDb.GroupCode = $scope.GroupCode;
        $scope.SaveDb.AccessLevel = $scope.AccessLevel;
        $scope.SaveDb.PostingLocation = $scope.frmUserCreationOPA.Employee.PostingLocation;
        $scope.SaveDb.DepotCode = $scope.frmUserCreationOPA.Employee.DepotCode;
        $scope.SaveDb.EmployeeCode = $scope.frmUserCreationOPA.Employee.EmployeeCode;

        //
        $http({
            method: "post",
            url: MyApp.rootPath + "UserCreationOPA/OperationsMode",
            datatype: "json",
    ...
    //
    $scope.GetEmployeeListOPA = function () {
        $http({
            method: "GET",
            url: MyApp.rootPath + "UserCreationOPA/GetActiveEmployeeInfoListOPA"
        }).then(function (response) {
            if (response.data !== "") {
    ...
    $scope.GetEmployeeListOPA();
    //
    $scope.Reset = function () {
        $scope.btnSaveValue = "Save";
        $scope.UserId = "";
        $scope.frmUserCreationOPA.Employee = "";
        $scope.Password = "";
        $scope.conPassword = "";
        $scope.Status = "";
        $scope.AccessLevel = "";
